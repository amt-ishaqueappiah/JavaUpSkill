...
public class CalculateMatrix {
    ...
    public void takeInputAndCalculate() {
        ...
        for (int i = 0; i < rowsA; i++) {
            matrixA[i] = getValidRowInput(colsA, "Enter values for row " + (i + 1) + " of matrix A (separated by spaces) it should be "+ colsA+" numbers for each row : ", mistakeCount);
        ...
        for (int i = 0; i < rowsB; i++) {
            matrixB[i] = getValidRowInput(colsB, "Enter values for row " + (i + 1) + " of matrix B (separated by spaces) it should be "+colsB+" numbers for each row : ", mistakeCount);
